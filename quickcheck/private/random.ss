...
(library (quickcheck private random)
  (export
  ...
  (define-record-type (random-generator really-make-random-generator random-generator?)
    (fields s1 s2))
    ...
    (let ([s1 (random-generator-s1 rg)]
      ...
      (let ([new-s1 (if (= s1 2147483562)
            ...
            [new-s2 (if (= s2 1)
                        2147483398
                        (- s2 1))])
        (let-values ([(_ nrg) (random-generator-next rg)])
  ...
  (define (random-integer rg low high)
    (let ([b 2147483561]
      ...
      (let loop ([n (ilogbase b k)]
                 ...
                 [rg rg])
        (if (zero? n)
            (values (+ low (modulo acc k)) rg)
            (let-values ([(x rgn) (random-generator-next rg)])
              (loop (- n 1) (+ x (* acc b)) rgn))))))

  (define (random-real rg low high)
    (let-values ([(x nrg) (random-integer rg min-bound max-bound)])
